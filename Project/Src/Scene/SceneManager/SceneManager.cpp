...
void SceneManager::RelocationCurrentCamera(void)
{
	// 1番上に重なっているシーンのカメラをCurrentCameraに設定する
	if (scenes.empty()) { CurrentCamera::Set(nullptr); }
	else {
		// 末尾から探索し、nullptrでないカメラが見つかるまで遡る
		for (std::size_t i = scenes.size() - 1; i >= 0; i--) {

			// そのシーンのカメラを取得する
			Camera* camera = scenes.at(i)->GetCamera();

			// nullptrでないカメラが見つかったらCurrentCameraに設定してループを抜ける
			if (camera != nullptr) { CurrentCamera::Set(scenes.at(i)->GetCamera()); break; }
		}
	}
}
