...
void TestScene::SubPostLoad(void)
{
	...
	// 操作対象（モデルあり）
	operatorObject = new Player(Vector3(0, 180, 0), true, true, true, 100, true);
	ObjAdd(operatorObject);
...
void TestScene::SubUiDraw(void)
{
	...
	DrawFormatString(
		...
		120,
		0xffff88,
		"空間制限の操作ボタン -> [1] 2Dモード  [2] 定点カメラ3Dモード  [3] 設定したレール上に縛るモード  [4] 完全フリーの3Dモード"
	);
